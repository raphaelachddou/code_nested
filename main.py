...
if args.traintime == True :
    ...
    if not(args.single):
        if not(args.FT):
            ...
            fine_model.fit(x_trains[2], y_trains[2],
                      ...
                      callbacks = callbacks,
                      validation_data=(x_vals[2], y_vals[2]))
            fine_model.save_weights('weights/model_multi_outputfine_tuned{}_id{}c{}m{}f{}.h5'.format(args.dataset,id,c, m,f))
    else:
        callbacks = [ReduceLROnPlateau(monitor = 'val_loss', factor = 0.2,patience =5),
        ...
        single_model.compile(loss = {"fine":'categorical_crossentropy'},loss_weights = {"fine":float(c)/f},optimizer= optimizer
                           ,metrics=['accuracy'])
        single_model.fit(x_trains[2], y_trains[1]['fine'],
                  batch_size=args.batch_size,
                  epochs=args.epochs,
                  verbose=1,
                  callbacks = callbacks,
                  validation_data=(x_vals[2], y_vals[1]['fine']))
    ...
    if not(args.single) :
        ...
        if args.FT == True:
            fine_model.load_weights('weights/model_multi_outputfine_tuned{}_id{}c{}m{}f{}.h5'.format(args.dataset,id,c,m,f), by_name = True)
        else :
            fine_model.load_weights('weights/model_multi_outputfine{}_id{}c{}m{}f{}.h5'.format(args.dataset,id,c,m,f),by_name = True)
