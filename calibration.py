...
_,_,x_tests,_,_,y_tests = data_processing(c/100.,m/100.,f/100.,args.perturbation,args.s,args.t)
print(x_test.shape)
print(x_tests.shape)
...
x_middle = x_train[:int(m*n/100.)]
x_fine = x_train[:int(f*n/100.)]

y_coarse,y_middle, y_fine = preprocessing_labels(y_train,c = c/100.,m = m/100., f = f/100. )
...
yc,ym,yf = preprocessing_labels(y_test,c = 1.,m = 1., f = 1., dataset = args.dataset )
...
if args.dataset =='cifar10':
    resc_2fine = 0.2*np.array([np.array([resc_scaled[i,0] for k in range(4)]+[resc_scaled[i,1] for k in range(6)]) for i in range(2000)])
    resm_2fine = 0.5*np.array([np.array([resm_scaled[i,0] for k in range(2)]+[resm_scaled[i,1] for k in range(2)]+[resm_scaled[i,2] for k in range(2)]+[resm_scaled[i,3]for k in range(2)]+[resm_scaled[i,4]for k in range(2)]) for i in range(2000)])
    resc_2fine_original = 0.2*np.array([np.array([resc_scaled_original[i,0] for k in range(4)]+[resc_scaled_original[i,1] for k in range(6)]) for i in range(2000)])
